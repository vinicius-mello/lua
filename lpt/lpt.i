...
inline char * hash(const lpt2d& r) {
	static char buffer[32];
	std::hash<lpt2d> h;
	sprintf(&buffer[0], "%016zx", h(r));
	return &buffer[0];
}
inline char * hash(const lpt3d& r) {
	static char buffer[32];
	std::hash<lpt3d> h;
	sprintf(&buffer[0], "%016zx", h(r));
	return &buffer[0];
...
class lpt2d {
    public:
        ...
        bool operator<(const lpt2d& r) const;
        bool operator==(const lpt2d& r) const;
};

char * hash(const lpt2d& r);
...
class lpt3d {
    public:
        ...
        void simplex(double * s) const;
        void print_simplex() const;
        bool operator<(const lpt3d& r) const;
        bool operator==(const lpt3d& r) const;
};
