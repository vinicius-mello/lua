...
double zeroTol = 0.00000001;

Complex ctheta1(Complex z, Complex q) {
  Complex p = 1;
  Complex s = 0;
	int i = 0;
  double sgn = 1;
  while (cabs(p) > zeroTol) {
		int t = 2*i+1;
    p = sgn*cpow(q, i*i+i)*csin(t*z);
    s = s+p;
    i++;
    sgn *= -1;
  }
  return 2*cpow(q, 0.25)*s;
}

Complex ctheta1d(Complex z, Complex q) {
  Complex p = 1;
  Complex s = 0;
	int i = 0;
  double sgn = 1;
...
Complex ctheta1dd(Complex z, Complex q) {
  Complex p = 1;
  Complex s = 0;
	int i = 0;
  double sgn = 1;
  while (cabs(p) > zeroTol) {
		int t = 2*i+1;
    p = sgn*cpow(q, i*i+i)*csin(t*z)*(-t*t);
    s = s+p;
    i++;
    sgn *= -1;
  }
...
Complex ctheta1ddd(Complex z, Complex q) {
  Complex p = 1;
  Complex s = 0;
	int i = 0;
  double sgn = 1;
  while (cabs(p) > zeroTol) {
		int t = 2*i+1;
    p = sgn*cpow(q, i*i+i)*ccos(t*z)*(-t*t*t);
    s = s+p;
    i++;
    sgn *= -1;
  }
...
Complex ctheta2(Complex z, Complex q) {
  Complex p = 1;
  Complex s = 0;
	int i = 0;
  while (cabs(p) > zeroTol) {
		int t = 2*i+1;
    p = cpow(q, i*i+i)*ccos(t*z);
    s = s+p;
    i++;
  }
  return 2*cpow(q, 0.25)*s;
...
Complex ctheta3(Complex z, Complex q) {
  Complex p = 1;
  Complex s = 0;
	int i = 1;
  while (cabs(p) > zeroTol) {
		int t = 2*i;
    p = cpow(q, i*i)*ccos(t*z);
    s = s+p;
    i++;
  }
  return 1+2*s;
...
Complex ctheta4(Complex z, Complex q) {
  Complex p = 1;
  Complex s = 0;
	int i = 1;
  while (cabs(p) > zeroTol) {
